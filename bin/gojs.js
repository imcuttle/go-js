...
if (opts.build) {
  ...
  const buildDirPath = nps.dirname(nps.resolve(opts.path, opts.build))
  const buildBaseName = nps.basename(nps.resolve(opts.path, opts.build))
  let buildTemplate = nps.join(buildDirPath, buildBaseName).replace(/\..+?$/, '.html')
  let tplString
  if (!fs.existsSync(buildTemplate)) {
    buildTemplate = defaultTemplatePath
    tplString = readFileSync(buildTemplate).toString()
  }
  else {
    tplString = readFileSync(buildTemplate).toString()
    const cheerio = require('cheerio')
    let $ = cheerio.load(tplString, {decodeEntities: false})
    $('body').append(`<script src="${ed.encode(opts.build)}.bundle.js"></script>`)
    tplString = $.html()
  }
